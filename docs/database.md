...
```
users ──1:N──> orders ──N:1──> schedules ──N:1──> routes
  │                │
  │                └──1:N──> order_rentals ──N:1──> rental_items
  │
  ├──1:N──> recharges (充值记录)
  ├──1:N──> referrals_inviter (我邀请的人)
  └──1:N──> referrals_invitee (谁邀请了我)
```
...
CREATE TABLE orders (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  order_no        VARCHAR(32) NOT NULL UNIQUE COMMENT '订单编号（年月日+序号）',
